...
export const TotalInput: FC<Props> = ({
  ...
  useEffect(() => {
    form.batch(() => {
      ...
      lineItems.forEach(({ quantity, unit_price, amount }, index) => {
        const quantitySource = `salesorderitem_set[${index}].quantity`;
        quantitySource !== formState.active &&
          form.change(quantitySource, quantity.toFixed());

        const unitPriceSource = `salesorderitem_set[${index}].unit_price`;
        unitPriceSource !== formState.active &&
          form.change(unitPriceSource, unit_price.toFixed(2));

        const amountSource = `salesorderitem_set[${index}].amount`;
  ...
  }, [amounts, discount_rate, form, formData.salesorderitem_set, gst_rate]);
